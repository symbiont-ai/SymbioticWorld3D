# Symbiotic Lab — session report 20260906_163041
...
|---|---|---|---|
| VC-1 | **MET** | Select a live Lumen and display its initial action-policy values. | per-organism Lumen policy data captured (6 live window(s), 0 ingested run(s)) [E-001, E-013, E-025] |
| VC-2 | **UNMET** | Let that same individual interact with the world for 30-60 seconds. | no per-lifetime Q-drift evidence ingested yet |
| VC-3 | **UNMET** | Show that its learned policy changed because of experience. | no mode B/C run in evidence with median lifetime Q drift >= 0.1 |
| VC-4 | **UNMET** | Show its offspring inheriting a related but mutated learning configuration. | no parent/child genome correlation in [0.5, 0.995] in evidence |
| VC-5 | **UNMET** | Accelerate time and show a population-level shift in meta-learning parameters. | no per-generation meta-parameter trend in evidence for mode C |
| VC-6 | **PARTIAL** | Trigger a drought and demonstrate adaptation continuing without offline retraining. | drought-proxy experiment(s) designed but not run yet: X-002, X-003, X-004 |
